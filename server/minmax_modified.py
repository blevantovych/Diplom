...
def pol(alternance, error_on_iteration, degree, f, start, end):
    ...
    eqs.append(make_eq(variables, start, f))
    for i in range(degree):
        eqs.append(make_eq(variables, alternance[i], f) + e)
        e *= -1
    eqs.append(make_eq(variables, end, f))
...
def main(f_str, start, end, degree, precision):
    ...
    del alternance[0]
    del alternance[-1]
    print 'initial alternance'
    print alternance
    iterations = 1

    pol_err_on_iter = pol(alternance, error_on_iteration, degree, f, start, end)
