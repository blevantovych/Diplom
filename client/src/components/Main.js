      ...
      <div className={classes.root}>
        ...
        >
          <Toolbar disableGutters={!open}>
            ...
            <Typography variant="h6" color="inherit" noWrap>
              {pathNameToTitle[this.props.location.pathname]}
            </Typography>
          </Toolbar>
        </AppBar>
        <Drawer
        ...
        >
          ...
          <List>
            {[
                {
                    text: 'Мінімаксний сплайн з заданою кількістю ланок',
                    path: '/spline-with-specified-number-of-segments'
                },
                {
                    text: 'Мінімаксний сплайн з заданою похибкою',
                    path: '/spline-with-precision'
                }
                
            ].map(({text, path}, index) => (
              <ListItem button key={text}>
                {/* <ListItemIcon>{index % 2 === 0 ? <InboxIcon /> : <MailIcon />}</ListItemIcon> */}
                <ListItemText primary={text} onClick={() => this.props.history.push(path)}/>
              </ListItem>
            ))}
          </List>
          {/* <Divider /> */}
